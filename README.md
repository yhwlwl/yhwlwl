...
你好，我是 yhwlwl，成都七中的一名高中生，也是 STA Network Department 的成员。我习惯把学习与生活中遇到的具体问题拆解成需求，再把它做成能真正运行的东西：从学习计划、校园文件服务，到数字水印研究，每一个项目都经历过"发现问题 → 设计 → 实现 → 部署 → 复盘改进"的完整过程。
...
- Study Planner 的计划调整与执行记录闭环；
- STA-PAN 系列平台的访问控制与阅读体验；
- DCT-Pro 水印在裁剪、压缩与几何变化下的鲁棒性实验。

## 说明

这里展示的项目都来自真实需求，处于"持续迭代"状态：有的已稳定服务内部使用，有的仍是研究原型或创意项目。所有 README 都尽量只写可验证的事实，不做夸大表述。欢迎浏览各仓库的 README 与文档索引，了解每个项目的具体边界。

## 联系

可以通过 GitHub 与我交流（Issues / Discussions），也可以在对应仓库中提交问题反馈。出于隐私考虑，这里不公开私人联系方式。
